...
from cybrscrape.core.utils import log
from cybrscrape.engines.toolbelt.custom import Response
from cybrscrape.core.utils._shell import _CookieParser, _ParseHeaders
from cybrscrape.core._types import List, Optional, Dict, Tuple, Any, Callable
...
try:
    ...
    raise ModuleNotFoundError(
        "You need to install cybrscrape with any of the extras to enable Shell commands. See: https://cybrscrape.readthedocs.io/en/latest/#installation"
    ) from e
...
def install(force):  # pragma: no cover
    if force or not __PACKAGE_DIR__.joinpath(".cybrscrape_dependencies_installed").exists():
        ...
        __PACKAGE_DIR__.joinpath(".cybrscrape_dependencies_installed").touch()
    else:
...
def shell(code, level):
    from cybrscrape.core.shell import CustomShell
...
def fetch(
    ...
    from cybrscrape.fetchers import DynamicFetcher
...
def stealthy_fetch(
    ...
    from cybrscrape.fetchers import StealthyFetcher
